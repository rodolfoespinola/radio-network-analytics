...
if df_pop is not None:
    pop_coberta_ano = []
    for ano in anos_validos:
        cids = set(df[df["Ano"]==ano]["Cidade_norm"].dropna().unique())
        pop  = df_pop[df_pop["Cidade_norm"].isin(cids)]["Populacao"].sum()
...
for i, ano in enumerate(anos_validos):
    radios_ano = set(df[df["Ano"] == ano]["Radio"])
...
impacto_radio = (
    df.groupby(["Radio","Cidade"])
    .agg(Veiculacoes_Cidade=("Identificador","count"), Populacao=("Populacao","first"))
    .reset_index()
    .groupby("Radio")
    .agg(Veiculacoes=("Veiculacoes_Cidade","sum"), Impacto=("Populacao","sum"),
         Cidades_Alcancadas=("Cidade","count"))
    .reset_index()
...
ax.bar(x_labels, df_churn["Novas"],  color=COR_POSITIVO, alpha=0.6, label="Novas Parceiras")
...
ax.set_xlabel("Impacto (população alcançada — M hab.)")
ax.set_title("Top 15 Rádios por Impacto Real* — Histórico Total", fontweight="bold")
ax.xaxis.set_major_formatter(mticker.FuncFormatter(lambda x,_: f"{x:.1f}M"))
nota_rodape(ax, "*Impacto = soma da população de cada cidade alcançada (sem duplicar por veiculação)")
fig.tight_layout()
...
if not lacunas.empty:
    ax.barh(lacunas["Municipio_IBGE"][::-1], lacunas["Populacao"][::-1]/1000, color=COR_ALERTA)
...
# ══ 6. EXPORTAÇÕES ══   ← linha existente, não mexer
...
if not fidelidade.empty:
    ...
    linhas += [f"  Rádios com presença contínua (100%):   {continuas}",
...
secao_extra = ["", "── DINÂMICA DE COBERTURA ────────────────────────────────────",
...
if "Pct_Pop_SC" in cobertura_ano.columns:
    ...
    linhas.append(f"  'Em {int(ult['Ano'])}, a programação alcançou municípios onde vivem "
